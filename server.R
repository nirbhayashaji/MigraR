...
server <- function(input, output, session) {
  ...
  observeEvent(input$tabset1,{
    if(input$tabset1 == "Velocity" || input$tabset1 == "Straightness" || input$tabset1 == "Direction"){
      hide("eixox")
      hide("eixoy")
      hide("radio")
    }
   ...
   if(input$slider[1] | input$sliderSpeed[1] | input$sliderStraight[1] | !is.null(input$trans)){
     if (input$radio == 1) {
       if (is.null(my_data()))
         return(NULL)
       if (input$trans != "All") 
         plot(filter(up_data(), up_data()$TrackID==input$trans)$Position.X, filter(up_data(), up_data()$TrackID==input$trans)$Position.Y, xlab="Distance (X)", ylab="Distance (Y)")
       else
         plot(up_data()$Position.X, up_data()$Position.Y, xlab="Distance (X)", ylab="Distance (Y)")
     }
     if (input$radio == 2) {
       if (is.null(my_data()))
         return(NULL)
       if (input$trans != "All") 
         plot(filter(up_data(), up_data()$TrackID==input$trans)$Position.X, filter(up_data(), up_data()$TrackID==input$trans)$Position.Y, col=filter(up_data(), up_data()$TrackID==input$trans)$TrackID, xlab="Distance (X)", ylab="Distance (Y)")
       else
         plot(up_data()$Position.X, up_data()$Position.Y, col=up_data()$TrackID, xlab="Distance (X)", ylab="Distance (Y)")
     ...
     if (input$radio == 3) {
       if (is.null(my_data()))
         ...
         plot(
           ...
           xlab = "Distance (um)",
           ylab = "Distance (um)",
           xlim = c(input$eixox[1], input$eixox[2]),
           ylim = c(input$eixoy[1], input$eixoy[2])
         )
         abline(v = 0)
         abline(h = 0)
       }
   ...
   if(input$slider[1] | input$sliderSpeed[1] | input$sliderStraight[1] | !is.null(input$trans)){ # | input$spaceTraveled| input$numPoints){
     ...
     if (input$radioCor == 2) {
         ...
         d<- ggplot(dados, aes(x=Posicao_x, y=Posicao_y, group=Track_ID , color=cos_angulo)) + 
           geom_point()  + labs(x = "Distance (um)", y = "Distance (um)", colour = "Direction (cos)")  + 
           ggtitle('Rose Plot')   + 
   ...
   if(input$slider[1] | input$sliderSpeed[1] | input$sliderStraight[1] | !is.null(input$trans)){ # | input$spaceTraveled) | input$numPoints){
     ...
     if (input$radioCor == 2) {
       ...
       d <- ggplot(dados, aes(x=Posicao_x, y=Posicao_y, group=Track_ID , color=cos_angulo)) + labs(x = "Distance (um)", y = "Distance (um)", colour = "Direction (cos)") +
         scale_colour_gradient2(low = "red",mid="orange", high = "yellow") + ggtitle('Normal View') +  theme(legend.position="bottom", legend.direction="horizontal") + geom_line()
         directionalityDataforDownload <<- up_data()[,14]
       print(d)
   ...
   if(input$slider[1] | input$sliderSpeed[1] | input$sliderStraight[1] | !is.null(input$trans)){ # | input$spaceTraveled| input$numPoints){
     if (input$radioCor == 2) {
       ...
       d<- ggplot(dados, aes(x=Posicao_x, y=Posicao_y, group=Track_ID , color=sin_angulo)) + 
         geom_point()  + labs(x = "Distance (um)", y = "Distance (um)", colour = "Direction (sin)")  + 
         ggtitle('Rose Plot')   + 
         theme(legend.position="bottom", legend.direction="horizontal") + 
         scale_x_continuous(limits=c(input$eixox1[1], input$eixox1[2])) + 
         scale_y_continuous(limits=c(input$eixoy1[1], input$eixoy1[2])) +
         scale_colour_gradient2(low = "blue",mid="aquamarine", high = "green")
   ...
   if(input$slider[1] | input$sliderSpeed[1] | input$sliderStraight[1] | !is.null(input$trans)){ # | input$spaceTraveled) | input$numPoints){
     ...
     if (input$radioCor == 2) {
       ...
       d <- ggplot(dados, aes(x=Posicao_x, y=Posicao_y, group=Track_ID , color=sin_angulo)) + labs(x = "Distance (um)", y = "Distance (um)", colour = "Direction (sin)") +
         scale_colour_gradient2(low = "blue",mid="aquamarine", high = "green") + ggtitle('Normal View') +  theme(legend.position="bottom", legend.direction="horizontal") + geom_line()
       directionalityDataforDownload <<- up_data()[,14]
       print(d)
...
dataProcess <- function(data){
  ...
  # Extraction of elements that have a Straightness equal to 1? maximum value 1 so how great than 1
  # _______________________________________________________________________________________
  ...
  # count is all 17770. hence why do this?
  ...
  for (i in 1:length(espaco_percorrido[,1]))
  {
    ...
    if (Track_ID_f==Track_ID)
    {
      num=num+as.numeric(as.character(espaco_percorrido[i,4])) #okay new track id on column 4 is used here
    }
    ...
    if (Track_ID != Track_ID_f && Track_ID_f != 0 || i==length(dados_filtrados[,1]))
    {
      ...
      if ((numero_de_ocorrencias + 1) > 0 && i==length(dados_filtrados[,1])) 
      {
        for (j in (numero_de_ocorrencias +1):0)
        {
          ...
          dados_filtrados_1[count,6]=dados_filtrados[i-j,6] 
          dados_filtrados_1[count,7]=as.numeric(as.character(dados_filtrados[i-j,7]))
          dados_filtrados_1[count,8]=dados_filtrados[i-j,8] # tis is making some changes
          dados_filtrados_1[count,9]=as.numeric(as.character(dados_filtrados[i-j,9]))
          dados_filtrados_1[count,10]=as.numeric(as.character(dados_filtrados[i-j,10]))
          dados_filtrados_1[count,11]=as.numeric(as.character(dados_filtrados[i-j,11]))
          dados_filtrados_1[count,12]=as.numeric(as.character(dados_filtrados[i-j,12]))
